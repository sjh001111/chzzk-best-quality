...
- 라이브 재생의 P2P/Grid 경로를 정리해 CDN 재생을 유도합니다.
- 광고 차단 경고가 뜨는 원인 플래그를 정리합니다.

## 동작 원리

치지직 VOD의 playback 응답에는 여러 화질 후보가 들어 있습니다. 이 스크립트는 재생 정보 응답을 읽을 때 1080p가 있으면 1080p 후보만 남기고, 없으면 사용 가능한 최고 화질 후보만 남겨 플레이어에 전달합니다.

라이브나 일부 HLS 재생에서 쓰는 `.m3u8` master playlist도 같은 기준으로 정리합니다.

라이브 재생 정보에 P2P/Grid 경로가 들어 있으면 해당 경로를 제거하고 일반 CDN 경로를 사용하도록 정리합니다.
